    ...
    <section className="relative overflow-hidden bg-background py-14 lg:py-20">
      ...
      <div className="relative mx-auto max-w-4xl px-5 lg:px-8">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-4 font-display text-4xl font-black uppercase leading-[0.85] tracking-tight text-balance md:text-6xl">
            <span className="block text-neon-blue text-glow">CHOW</span>
            <span className="block">Winners</span>
          </h2>
          <p className="mx-auto mt-4 max-w-xl text-pretty leading-relaxed text-light-grey">
          ...
          {challenge.trim() ? (
            <div className="mx-auto mt-5 inline-flex flex-col items-center gap-1 rounded-2xl border border-neon-green/50 bg-neon-green/10 px-6 py-3 green-glow">
        ...
        <div className="mt-8 flex flex-col gap-3">
          {winners.map((w, i) => (
            <Reveal key={w.id} delay={i * 120}>
              <div className="group flex items-center gap-4 rounded-2xl border border-neon-blue/50 bg-black/40 p-4 backdrop-blur transition-all hover:border-neon-blue hover:blue-glow sm:p-5">
                {w.imageUrl ? (
                  <span className="relative size-12 shrink-0 overflow-hidden rounded-full border border-neon-blue/60 sm:size-14">
                    <Image src={w.imageUrl || '/placeholder.svg'} alt={w.name} fill className="object-cover" sizes="56px" />
                  </span>
                ) : (
                  <span className="flex size-12 shrink-0 items-center justify-center rounded-full border border-neon-blue/60 bg-cobalt/15 text-neon-blue transition-colors group-hover:bg-cobalt group-hover:text-accent-foreground sm:size-14">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-display text-3xl font-black uppercase leading-none tracking-tight text-foreground sm:text-4xl">
                    {w.name}
                  </p>
                  {w.achievement ? (
                ...
                {w.score.trim() ? (
                  <div className="shrink-0 text-right">
                    <p className="font-display text-3xl font-black leading-none tracking-tight text-neon-green text-glow sm:text-4xl">
                      {w.score}
                    </p>
        ...
        <p className="mt-8 text-center font-display text-lg font-bold uppercase tracking-tight text-foreground">
