...
export function TeamLogoUploader({
  ...
  async function save() {
    ...
    try {
      ...
      fd.append("file", new File([blob], "team-logo.png", { type: "image/png" }))
      const res = await fetch("/api/upload/team-logo", { method: "POST", body: fd })
      ...
      {!editing && (
        <div className="flex items-center gap-3">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-md border border-border bg-secondary">
            {previewUrl ? (
              <Image src={previewUrl || "/placeholder.svg"} alt="Team logo" width={64} height={64} className="h-full w-full object-cover" />
            ) : (
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {value ? (
              <Button type="button" variant="ghost" size="sm" className="gap-2 text-muted-foreground" onClick={() => onChange("")}>
                <X className="h-4 w-4" /> Use venue logo
              </Button>
            ) : null}
          </div>
        </div>
