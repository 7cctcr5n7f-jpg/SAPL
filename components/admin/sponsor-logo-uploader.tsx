...
import { ImageIcon, Upload, Crop as CropIcon, Loader2, X } from "lucide-react"
...
const ASPECTS: Aspect[] = [
  { key: "wide", label: "Wide", ratio: 16 / 6, out: [640, 240] },
  { key: "square", label: "Square", ratio: 1, out: [400, 400] },
]
...
export function SponsorLogoUploader({
  ...
  async function save() {
    ...
    try {
      ...
      fd.append("file", new File([blob], "sponsor.png", { type: "image/png" }))
    ...
    <div className="space-y-3">
      ...
      {!editing && (
        <div className="flex items-center gap-3">
          <div className="flex h-16 w-28 shrink-0 items-center justify-center overflow-hidden rounded-md border border-border bg-secondary">
            {value ? (
              <Image src={value || "/placeholder.svg"} alt="Sponsor logo" width={112} height={64} className="h-full w-full object-contain" />
            ) : (
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="outline" size="sm" className="gap-2" onClick={() => fileRef.current?.click()}>
              <Upload className="h-4 w-4" /> {value ? "Replace logo" : "Upload logo"}
            </Button>
            {value ? (
      ...
      {editing && src && (
        <div className="space-y-3 rounded-lg border border-border bg-card p-3">
          <div
            ...
            style={{ aspectRatio: String(aspect.ratio), maxWidth: aspect.ratio === 1 ? 280 : 480 }}
          >
            {stretch ? (
          ...
          <p className="text-center text-[11px] uppercase tracking-wider text-muted-foreground">Preview of the saved logo</p>
          ...
          <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Fit the whole logo into the frame, ignoring proportions</p>
            </div>
